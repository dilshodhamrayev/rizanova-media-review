...
export default function Music(props) {
    ...
    const { data } = useSWR(`music/premiere?page=${page}`, get, { initialData: props.data });

    const title = t("Премьеры узбекских треков");
    const description = t("Премьера узбекских треков на сайте RizaNova. Здесь вы можете слушать последние премьеры треков в хорошем качестве.");
    const keywords = t("премьеры последние новые узбекских треки музыки певец певицы");
            ...
            <div className="container min-view">
                ...
                <div className="row">
                    ...
                    <div className="col-md-9">
                        <h1 className="title-2 mt-0 mb-4 pb-2">{t("Музыка")}</h1>
                            ...
                            {data.models.map((model, index) => {
                                return (
                                    <div className="col-lg-3 col-md-4 col-4 mb-4" key={index}>
                                        <MusicItem model={model} hidePremiere />
                                    </div>
                                );
                            })}
                        </div>
...
export const getServerSideProps = withSession(async function ({ req, res, query: { page = 1 }, locale }) {
    ...
    try {
        let data = await get(`music/premiere?page=${page}`, params);
