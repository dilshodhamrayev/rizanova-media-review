...
export default function Film(props) {
    ...
    const { data } = useSWR(`film/premiere?page=${page}`, get, { initialData: props.data });

    const title = t("Премьеры узбекских фильмов");
    const description = t("Премьера узбекских фильмов на сайте RizaNova. Здесь вы можете смотреть последние премьеры фильмов в хорошем качестве.");
            ...
            <div className="container min-view inner-page">
                ...
                <div className="row">
                    ...
                    <div className="col-md-9">
                        <h1 className="title-2 mt-0 mb-4 pb-2">{t("Фильмы")}</h1>
                            ...
                            {data.models.map((model, index) => {
                                return (
                                    <div className="col-lg-4 col-md-4 col-6 mb-4" key={index}>
                                        <FilmItem link="film" model={model} hidePremiere />
                                    </div>
                                );
                            })}
                        </div>
...
export const getServerSideProps = withSession(async function ({ req, res, query: { page = 1 }, locale }) {
    ...
    try {
        let data = await get(`film/premiere?page=${page}`, params);
