...
export default function Serial(props) {
    ...
    const { data, mutate } = useSWR(`account/history?type=serial&page=${page}`, get, { initialData: props.data });

    const clearHistory = async () => {
        let res = await get('account/clear-history?type=serial');
...
export const getServerSideProps = withSession(async function ({ req, res, query: { page = 1 }, locale }) {
    ...
    try {
        ...
        let data = await get(`account/history?type=serial&page=${page}`, params);
