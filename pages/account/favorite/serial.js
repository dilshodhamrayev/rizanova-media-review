...
export default function Serial(props) {
    ...
    const { data } = useSWR(`account/serial?page=${page}`, get, { initialData: props.data });
            ...
            <div className="container min-view inner-page">
                ...
                <div className="row">
                    ...
                    <div className="col-md-9">
                        ...
                        {data.models.length === 0 && <Empty text={t("Здесь пока пусто, Вы ещё не отметили видео как понравившиеся.")} />}
                        <div className="row border-radius-4 padding-0 ">

                            {data.models.map((model, index) => {
                                return (
                                    <div className="col-lg-4 col-md-6 col-6 mb-4" key={index}>
                                        <FilmItem model={model} hidePremiere className="px-0" link="serial" />
                                    </div>
                                );
                            })}
                        </div>
...
export const getServerSideProps = withSession(async function ({ req, res, query: { page = 1 }, locale }) {
    ...
    try {
        ...
        let data = await get(`account/serial?page=${page}`, params);
