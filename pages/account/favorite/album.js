...
export default function Clip(props) {
    ...
    const { data } = useSWR(`account/album?page=${page}`, get, { initialData: props.data });
            ...
            <div className="container min-view inner-page">
                ...
                <div className="row">
                    ...
                    <div className="col-md-8 col-lg-9">
                        <AccountFavoriteNavbar />

                        {data.models.length === 0 && <Empty text={t("Здесь пока пусто, Вы ещё не отметили альбом как понравившиеся.")} />}
                        <div className="row border-radius-4 padding-0 ">

                            {data.models.map((model, index) => {
                                return (
                                    <div className="col-xl-3 col-lg-4 col-md-6 col-6 mb-4" key={index}>
                                        <AlbumItem model={model} hidePremiere className="px-0" />
                                    </div>
                                );
                            })}
                        </div>
...
export const getServerSideProps = withSession(async function ({ req, res, query: { page = 1 }, locale }) {
    ...
    try {
        ...
        let data = await get(`account/album?page=${page}`, params);
