...
export default function View(props) {
    ...
    const { data: { pagination, models, genre } } = useSWR(`film/genre?genre_id=${genre_id}&page=${page}`, get, { initialData: props.data });
    ...
    return <div>
        ...
        <div className="container min-view inner-page">
            <ul className="breadcrumbs">
                ...
                <li><span>/</span></li>
                <li><Link href="/film/genre"><a>{t("Жанры")}</a></Link></li>
                <li><span>/</span></li>
                <li><p>{genre['name_' + i18n.language]}</p></li>
            </ul>

            <h1 className="big-header">
                {t("Фильмы по жанру")} «<Link href="/film/genre"><a>{genre['name_' + i18n.language]}</a></Link>»
            </h1>
...
export const getServerSideProps = withSession(async function ({ req, res, query: { genre_id, page = 1 }, locale }) {
    ...
    try {
        let data = await get(`film/genre?genre_id=${genre_id}&page=${page}`, params);
