...
export default function RizanovaRecommend(props) {
    ...
    const { data: { pagination, models } } = useSWR(`film/rizanova-recommend?page=${page}`, get, { initialData: props.data });

    const { store: { audioPlayer }, dispatch } = useContext(GlobalContext);

    const title = t("Рекомендуемые фильмы");
    const description = t("Рекомендуемые фильмы сайте RizaNova. Онлайн-кинотеатр узбекских фильмов в хорошем качестве только на сайте RizaNova.");
...
export const getServerSideProps = withSession(async function ({ req, res, query: { page = 1 }, locale }) {
    ...
    try {
        let data = await get(`film/rizanova-recommend?page${page}`, params);
