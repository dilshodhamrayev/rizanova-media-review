...
export default function FilmCast(props) {
    ...
    const { data: model } = useSWR(`film/persons?id=${cast_film_id}`, get, { initialData: props.data });

    const title = model.name + " — " + t("Актёры и создатели фильма");
        ...
        <div>
            ...
            <div className="container min-view pb-lg-5 mb-lg-5">
                <ul className="breadcrumbs">
                    ...
                    <li><span>/</span></li>
                    <li><Link href={`/film/${model.id}`}><a>{model.name}</a></Link></li>
                    <li><span>/</span></li>
                    <li><p>{t("Актёры и создатели фильма")}</p></li>
                </ul>

                <h1 className="big-header">{t("Актёры и создатели фильма")} «<Link href={`/film/${model.id}`}><a>{model.name}</a></Link>»</h1>
...
export const getServerSideProps = withSession(async function ({ req, res, query: { cast_film_id }, locale }) {
    ...
    try {
        let data = await get(`film/persons?id=${cast_film_id}`, params);
