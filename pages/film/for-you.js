...
export default function ForYou(props) {
    ...
    const { data: { pagination, models } } = useSWR(`film/for-u?page=${page}`, get, { initialData: props.data });

    const { store: { audioPlayer }, dispatch } = useContext(GlobalContext);

    const title = t("Фильмы для вас");
    const description = t("Сайт RizaNova рекомендует вам фильмы на основе ваших историй о просмотре фильмов. Онлайн-кинотеатр узбекских фильмов в хорошем качестве только на сайте RizaNova.");
...
export const getServerSideProps = withSession(async function ({ req, res, query: { page = 1 }, locale }) {
    ...
    try {
        let data = await get(`film/for-u?page${page}`, params);
