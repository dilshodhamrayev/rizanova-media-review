...
export default function CategoryView(props) {
    ...
    const { data: { model, models } } = useSWR(`category/get?id=${category_id}`, get, { initialData: props.category });
    const { data: { models: musics, pagination } } = useSWR(`music/index?category_id=${category_id}&page=${page}`, get, { initialData: props.musics });

    let seconds = musics.length > 0 ? musics.map(music => toSeconds(music.duration)).reduce((a, b) => a + b) : 0;

    let hours = Math.floor(moment.duration(seconds, 'seconds').asHours());
    let minutes = Math.floor(moment.duration(seconds, 'seconds').asMinutes());
    ...
    const play = (music_id) => {
        if (audioPlayer.currentPlaylist_id === "category_" + model.id && (music_id === audioPlayer.currentTrackId || !music_id))
            ...
            if (musics.length > 0)
                dispatch({ type: TYPE_AP_SET_TRACKS, payload: musics, currentTrackId: music_id ? music_id : musics[0].id, currentPlaylist_id: "category_" + model.id, currentPlaylist: model.name });
        }
    }
                        ...
                        <div className="details">
                            <div className="d-flex">
                                ...
                                <div className="info">
                                    ...
                                    <p className="tracks-info">{musics.length} {t("треков")} • {t("Длительность")} {hours > 0 ? hours + ' ' + t('ч.') : ''} {minutes} {t("мин.")}</p>
                                    <div className="d-flex">
                                        {musics.length > 0 && <a role="button" className="btn-spec me-3" onClick={() => {
                                            play();
                                        }}>
                            ...
                            {musics.length > 0 ? <>
                                ...
                                <div className="album-music-list">
                                    {musics.map((music, index) => {
                                        return (
                                            <SingleMusicItem model={music} order={(page - 1) * 12 + (index + 1)} key={index} models={musics} />
                                        );
                                    })}
                                </div>
                                <Pagination pathname={`/music/category/${model.id}`} pageCount={pagination.pageCount || 0} totalCount={pagination.totalCount || 0} initialPage={page - 1} />
                            </> : (
...
export const getServerSideProps = withSession(async function ({ req, res, query: { category_id, page = 1 }, locale }) {
    ...
    try {
        let [category, musics] = await Promise.all([
            get(`category/get?id=${category_id}`, params),
            get(`music/index?category_id=${category_id}&page=${page}`, params),
        ]);

        if (category.status == 'not-found') {
            return {
                props: {
                    category,
                    musics,
