...
export default function View(props) {
    ...
    const { person_films } = router.query;

    const { data: { model, models } } = useSWR(`film/person?id=${person_films}`, get, { initialData: props.data });

    const { store: { audioPlayer }, dispatch } = useContext(GlobalContext);

    const title = model.name + ' — ' + t("фильмы");
    ...
    return <div>
        ...
        <div className="container min-view inner-page">
            ...
            <h1 className="big-header pb-4 mb-2 d-flex">
                {t("Фильмы персона")}: <Select model={model} />
            </h1>
...
export const getServerSideProps = withSession(async function ({ req, res, query: { person_films }, locale }) {
    ...
    try {
        let data = await get(`film/person?id=${person_films}`, params);
