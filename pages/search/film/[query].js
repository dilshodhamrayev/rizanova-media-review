...
export default function Search(props) {
    ...
    const { data: films } = useSWR(`film/index?q=${encodeURIComponent(query)}&page=${page}`, get, { initialData: props.films });
    ...
    return (
        <div className="search-page">
            ...
            <div className="container min-view inner-page">
                    ...
                    <div className="col-lg-8">
                        <div className={'search-form w-100'}>
                            <form onSubmit={(e) => {
                                ...
                                if (value.length > 1) {
                                    router.push(`/search/film/${encodeURIComponent(value)}`);
                                } else {
                ...
                <Pagination pageCount={films.pagination.pageCount || 0} totalCount={films.pagination.totalCount || 0} initialPage={page - 1} pathname={`/search/film/${encodeURIComponent(query)}`} />
            </div>
        </div>
    );
}
...
export const getServerSideProps = withSession(async function ({ req, res, query: { query, page = 1 }, locale }) {
    ...
    try {
        let [films] = await Promise.all([
            get(`film/index?q=${encodeURIComponent(query)}&page=${page}`, params),
        ]);
