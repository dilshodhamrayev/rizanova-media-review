...
export default function Search(props) {
    ...
    const { data: films } = useSWR(`telenovella/index?q=${encodeURIComponent(query)}&page=${page}`, get, { initialData: props.films });

    const title = t("Поиск фильмов по сайту RizaNova");
    ...
    return (
        <div className="search-page">
            ...
            <div className="container min-view inner-page">
                    ...
                    <div className="col-lg-8">
                        <div className={'search-form w-100'}>
                            <form onSubmit={(e) => {
                                ...
                                if (value.length > 1) {
                                    router.push(`/search/telenovella/${encodeURIComponent(value)}`);
                                } else {
                ...
                <h1 className="title-2 mt-0">{t("Фильмы по запросу")} <span className="font-weight-400">«{query}»</span></h1>

                {films.models.length == 0 && <h1 className="title-2 mt-0 text-center">{t("Ничего не найдено")}</h1>}
                <div className="row">
                    {films.models.map((film, index) => {
                        return (
                ...
                <Pagination pageCount={films.pagination.pageCount || 0} totalCount={films.pagination.totalCount || 0} initialPage={page - 1} pathname={`/search/telenovella/${encodeURIComponent(query)}`} />
            </div>
        </div>
    );
}
...
export const getServerSideProps = withSession(async function ({ req, res, query: { query, page = 1 }, locale }) {
    ...
    try {
        let [films] = await Promise.all([
            get(`telenovella/index?q=${encodeURIComponent(query)}&page=${page}`, params),
        ]);
