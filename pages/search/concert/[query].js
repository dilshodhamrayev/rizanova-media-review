...
export default function Search(props) {
    ...
    const { data: concerts } = useSWR(`concert/index?q=${encodeURIComponent(query)}&page=${page}`, get, { initialData: props.concerts });

    const title = t("Поиск концертов по сайту RizaNova");
    ...
    return (
        <div className="search-page">
            ...
            <div className="container min-view inner-page">
                    ...
                    <div className="col-lg-8">
                        <div className={'search-form w-100'}>
                            <form onSubmit={(e) => {
                                ...
                                if (value.length > 1) {
                                    router.push(`/search/concert/${encodeURIComponent(value)}`);
                                } else {
                ...
                <h1 className="title-2 mt-0">{t("Концерты по запросу")} <span className="font-weight-400">«{query}»</span></h1>
                {concerts.models.length == 0 && <h1 className="title-2 mt-0 text-center">{t("Ничего не найдено")}</h1>}

                <div className="row">
                    {concerts.models.map((film, index) => {
                        return (
                            <div className="col-6 col-md-4 col-lg-3 mb-4" key={index}>
                                <ClipItem model={film} hidePremiere className="px-0" link="concert" />
                            </div>
                        );
                    })}
                </div>

                <Pagination pageCount={concerts.pagination.pageCount || 0} totalCount={concerts.pagination.totalCount || 0} initialPage={page - 1} pathname={`/search/concert/${encodeURIComponent(query)}`} />
            </div>
        </div>
    );
}
...
export const getServerSideProps = withSession(async function ({ req, res, query: { query, page = 1 }, locale }) {
    ...
    try {
        let [concerts] = await Promise.all([
            get(`concert/index?q=${encodeURIComponent(query)}&page=${page}`, params),
        ]);
