...
export default function Search(props) {
    ...
    const { data: musics } = useSWR(`music/index?q=${encodeURIComponent(query)}&page=${page}`, get, { initialData: props.musics });

    const title = t("Поиск треков по сайту RizaNova");
    ...
    return (
        <div className="search-page">
            ...
            <div className="container min-view inner-page">
                    ...
                    <div className="col-lg-8">
                        <div className={'search-form w-100'}>
                            <form onSubmit={(e) => {
                                ...
                                if (value.length > 1) {
                                    router.push(`/search/music/${encodeURIComponent(value)}`);
                                } else {
                ...
                <h1 className="title-2 mt-0">{t("Музыки по запросу")} <span className="font-weight-400">«{query}»</span></h1>
                {musics.models.length == 0 && <h1 className="title-2 mt-0 text-center">{t("Ничего не найдено")}</h1>}

                <div className="row">
                    {musics.models.map((film, index) => {
                        return (
                            <div className="col-4 col-md-4 col-lg-3 mb-4" key={index}>
                                <MusicItem model={film} hidePremiere className="px-0" models={musics.models} />
                            </div>
                        );
                    })}
                </div>

                <Pagination pageCount={musics.pagination.pageCount || 0} totalCount={musics.pagination.totalCount || 0} initialPage={page - 1} pathname={`/search/music/${encodeURIComponent(query)}`} />
            </div>
        </div>
    );
}
...
export const getServerSideProps = withSession(async function ({ req, res, query: { query, page = 1 }, locale }) {
    ...
    try {
        let [musics] = await Promise.all([
            get(`music/index?q=${encodeURIComponent(query)}&page=${page}`, params),
        ]);
