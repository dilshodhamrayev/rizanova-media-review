...
export default function WatchingNow(props) {
    ...
    const title = t("Сериалы смотрят сейчас");
    const description = t("Популярные сериалы на сегодня на сайте RizaNova. Онлайн-кинотеатр узбекских сериалов в хорошем качестве только на сайте RizaNova.");
...
export const getServerSideProps = withSession(async function ({ req, res, query: { page = 1 }, locale }) {
    ...
    try {
        let data = await get(`serial/watching-now?page${page}`, params);
