...
export default function AllSerial(props) {
    ...
    const { data: { pagination, models, max_year, min_year } } = useSWR(`serial/index?page=${page}&limit=16&sort=${sort}&year=${year}`, get, { initialData: props.data });

    const title = t("Все сериалы");
    const description = t("Все узбекские сериалы на сайте RizaNova. Онлайн-кинотеатр узбекских фильмов в хорошем качестве только на сайте RizaNova.");
    ...
    return <div>
        ...
        <div className="container min-view inner-page">
            <ul className="breadcrumbs">
                ...
                <li><span>/</span></li>
                <li><p>{t("Все сериалы")}</p></li>
            </ul>

            <div className="d-flex justify-content-between">
                <h1 className="big-header d-flex align-items-center">{t("Все сериалы")}</h1>
...
export const getServerSideProps = withSession(async function ({ req, res, query: { page = 1, sort = "newest" }, locale }) {
    ...
    try {
        let data = await get(`serial/index?page=${page}&limit=16&sort=${sort}`, params);
