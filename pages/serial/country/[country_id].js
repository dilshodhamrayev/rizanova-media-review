...
export default function View(props) {
    ...
    const { data: { pagination, models, country } } = useSWR(`serial/country?country_id=${country_id}&page=${page}`, get, { initialData: props.data });

    const { store: { audioPlayer }, dispatch } = useContext(GlobalContext);

    const title = t("Сериалы страны") + " — " + country['name_' + i18n.language];
    ...
    return <div>
        ...
        <div className="container min-view inner-page">
            <ul className="breadcrumbs">
                ...
                <li><span>/</span></li>
                <li><p>{country['name_' + i18n.language]}</p></li>
            </ul>

            <h1 className="big-header">
                {t("Сериалы страны")} «{country['name_' + i18n.language]}»
            </h1>
...
export const getServerSideProps = withSession(async function ({ req, res, query: { country_id, page = 1 }, locale }) {
    ...
    try {
        let data = await get(`serial/country?country_id=${country_id}&page=${page}`, params);
