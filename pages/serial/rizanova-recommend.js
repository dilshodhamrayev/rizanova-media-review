...
export default function RizanovaRecommend(props) {
    ...
    const { data: { pagination, models } } = useSWR(`serial/watching-now?page=${page}`, get, { initialData: props.data });

    const { store: { audioPlayer }, dispatch } = useContext(GlobalContext);

    const title = t("Рекомендуемые сериалы");
    const description = t("Рекомендуемые сериалы сайте RizaNova. Онлайн-кинотеатр узбекских сериалов в хорошем качестве только на сайте RizaNova.");
    ...
    return <div>
        ...
        <div className="container min-view inner-page">
            <ul className="breadcrumbs">
                ...
                <li><span>/</span></li>
                <li><p>{t("RizaNova рекомендует")}</p></li>
            </ul>

            <h1 className="big-header">
                {t("RizaNova рекомендует")}
            </h1>
...
export const getServerSideProps = withSession(async function ({ req, res, query: { page = 1 }, locale }) {
    ...
    try {
        let data = await get(`serial/rizanova-recommend?page${page}`, params);
