...
export default function RegisterForm({ showLogin, closeModal, isModal = false }) {
    ...
    const onSubmit = async data => {
        try {
            ...
            let { data: res } = await axios.post('/api/signup', data);

            if (res.status === 'error') {
                if (res.code == 1) {
                    toast.error(t("Номер телефона, пароль или Ф.И.О. не введены"), { toastId: "register-toast", updateId: "register-toast" });
                } else if (res.code == 4) {
                    toast.error(requiredPhone ? t("Этот номер телефона уже занят") : t("Эта почта уже занята"), { toastId: "register-toast", updateId: "register-toast" })
                } else {
                    toast.error(t("Ошибка валидации"), { toastId: "register-toast", updateId: "register-toast" });
                }
                ...
                if (res.user.status == 10) {
                    ...
                    toast.info((requiredPhone ? t("Пожалуйста, подтвердите номер телефона") : t("Пожалуйста, подтвердите почту")) + ": " + res.user.username, { toastId: "register-toast", updateId: "register-toast" });
                    setTimeLeft(180);
                    setStep(2);
                    setUser(res.user);
                }
    ...
    const onConfirm = async (data) => {
        ...
        try {
            ...
            let { data: res } = await axios.post('/api/confirm', {
                code: data.code,
                id: user.id
            });
            ...
            if (res.status === 'error') {
                ...
                toast.success(requiredPhone ? t("Вы успешно подтвердили ваш номер") : t("Вы успешно подтвердили вашу почту"), { toastId: "register-toast", updateId: "register-toast" });
            ...
            {step == 1 ? (
                <>
                    <div className="d-flex justify-content-center w-100">
                        <h3 className="mb-0">{t("Регистрация")}</h3>
                    </div>
                    <div className="d-flex justify-content-center w-100">
                        <p className="have-an-account my-0">
                            {t("Уже есть аккаунт?")}
                        </p>
                    </div>
                    <div className="d-flex justify-content-center w-100">
                        {isModal ? (
                            <a role="button" className="login-text-btn" onClick={showLogin}>{t("Войти")}</a>
                        ) : (
                            <Link href="/login">
                                <a className="login-text-btn">{t("Войти")}</a>
                            </Link>
                        )}
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="form-group">
                            <div>
                                <input type='text' {...register("full_name", { required: true })} className={errors.full_name ? 'has-error' : ''} placeholder={t("Введите Ф.И.О.") + "*"} />
                            </div>
                        </div>
                        {requiredPhone ? <>
                            <div className="form-group">
                                <div>
                                    <InputMask
                                        ...
                                        placeholder={t("Введите номер телефона") + (requiredPhone ? "*" : "")}
                                        control={control}
                                        name="username"
                                        rules={{
                                    ...
                                    <a role="button" className='no-phone-btn' onClick={() => setRequiredPhone(false)}>{t("У меня нет телефона")}</a>
                                </div>
                            </div>
                            <div className="form-group">
                                <div>
                                    <input type="text" {...register("email", { required: false, pattern: /^\S+@\S+$/i })} className={errors.email ? 'has-error' : ''} placeholder={t("Введите почту")} />
                                    {errors.email && errors.email.message.length > 0 && <p className="error-block">{errors.email.message}</p>}
                                </div>
                            </div>
                        </> : <>
                            <div className="form-group">
                                <div>
                                    <input type="text" {...register("username", { required: !requiredPhone, pattern: /^\S+@\S+$/i })} className={errors.username ? 'has-error' : ''} placeholder={t("Введите почту") + "*"} />
                                    {errors.username && errors.username.message.length > 0 && <p className="error-block">{errors.username.message}</p>}
                                    <a role="button" className='no-phone-btn' onClick={() => setRequiredPhone(true)}>{t("Ввести номер телефона")}</a>
                                </div>
                            </div>
                        </>}
                        <div className="form-group position-relative mb-0">
                            ...
                            })} className={errors.password ? 'has-error' : ''} placeholder={t("Введите пароль") + "*"} />
                        ...
                        {errors.password && errors.password.message.length > 0 && <p className="error-block">{errors.password.message}</p>}
                        <div className="form-group mb-0 mt-3 position-relative">
                            <input type={showPassword ? 'text' : "password"} {...register("repeat_password", {
                                required: true,
                                validate: value => value === password.current || t("Пароли не совпадают")
                            })} className={errors.repeat_password ? 'has-error' : ''} placeholder={t("Повторите пароль") + "*"} />
                            <a role="button" className={"eye " + (showPassword ? 'active' : '')} onClick={() => setShowPassword(!showPassword)}>
                                {/* Eye svg */}
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="14" viewBox="0 0 20 14">
                                    <path d="M10.0007 0.75C5.83398 0.75 2.27565 3.34167 0.833984 7C2.27565 10.6583 5.83398 13.25 10.0007 13.25C14.1673 13.25 17.7256 10.6583 19.1673 7C17.7256 3.34167 14.1673 0.75 10.0007 0.75ZM10.0007 11.1667C7.70065 11.1667 5.83398 9.3 5.83398 7C5.83398 4.7 7.70065 2.83333 10.0007 2.83333C12.3006 2.83333 14.1673 4.7 14.1673 7C14.1673 9.3 12.3006 11.1667 10.0007 11.1667ZM10.0007 4.5C8.61732 4.5 7.50065 5.61667 7.50065 7C7.50065 8.38333 8.61732 9.5 10.0007 9.5C11.384 9.5 12.5007 8.38333 12.5007 7C12.5007 5.61667 11.384 4.5 10.0007 4.5Z" />
                        ...
                        {errors.repeat_password && errors.repeat_password.message.length > 0 && <p className="error-block">{errors.repeat_password.message}</p>}

                        <div>
                            <button type="submit" className="submit" disabled={isLoading}>{
                                isLoading ? <i className="fa fa-spinner rotating"></i> : t("Зарегистрироваться")
                            }</button>
                        </div>
                    </form>
                </>
                ...
                <>
                    <div className="d-flex justify-content-center w-100">
                        <h3 className="mb-0">{t("Подтверждение номер телефона")}</h3>
                    </div>
                    <div className="d-flex justify-content-center w-100">
                            ...
                            {timeLeft > 0 ? (
                                ...
                                <a role="button" className="remember-me" onClick={async () => {
                                    ...
                                    if (res.status == "success") {
                                        setTimeLeft(180);
                                        toast.info((requiredPhone ? t("Повторный код отправлен на номер") : t("Повторный код отправлен на почту")) + ": " + user.username)
                                    }
