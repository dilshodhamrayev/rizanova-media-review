...
export default function LoginForm({ showRegister, closeModal, isModal = false }) {
        ...
        try {
            ...
            if (res.status === 'error') {
                if (res.code == 1) {
                    ...
                    toast.error(t("Логин или пароль не введены"), { toastId: "login-toast", updateId: "login-toast" });
                } else {

                    toast.error(t("Логин или пароль введены не верно"), { toastId: "login-toast", updateId: "login-toast" });
                }
            } else if (res.status == 'success') {
                toast.success(t("Вход успешно выполнен"), { toastId: "login-toast", updateId: "login-toast" });
    ...
    const onRestore = async data => {
        try {
            ...
            if (step == 2) {
                ...
                if (res.status === 'error') {
                    if (res.code == 1) {
                        toast.error(t("Номер не отправлен"), { toastId: "login-toast", updateId: "login-toast" });
                    } else if (res.code == 2) {
                        toast.error(t("Пользователь не найден"), { toastId: "login-toast", updateId: "login-toast" });
                    } else {
                        toast.error(t("Номер не отправлен"), { toastId: "login-toast", updateId: "login-toast" });
                    }
                } else if (res.status == 'success') {
                    toast.info((data.username.indexOf("@") > -1 ? t("Пожалуйста, подтвердите почту") : t("Пожалуйста, подтвердите номер телефона")) + ": " + data.username, { toastId: "login-toast", updateId: "login-toast" });
                    setTimeLeft(180);
                    setStep(3);
                }
            } else if (step == 3) {
                ...
                if (res.status == "error") {
                    ...
                    toast.success(t("Введите новый пароль"));
                    setStep(4);
                }
            } else if (step == 4) {
                ...
                if (res.status == "error") {
                    toast.error(t("Ошибка при подтверждения кода"))
                } else {
                    toast.success(t("Новый пароль успешно сохранён"));
                    setStep(1);
                }
            }
    ...
    const socialLogin = async (data) => {
        try {
            ...
            if (res.status === 'error') {
                if (res.code == 1) {
                    toast.error(t("Источник не указан"), { toastId: "login-toast", updateId: "login-toast" });
                } else {
                    toast.error(t("Авторизация не удалась"), { toastId: "login-toast", updateId: "login-toast" });
                }
            } else if (res.status == 'success') {
                toast.success(t("Вход успешно выполнен"), { toastId: "login-toast", updateId: "login-toast" });
            ...
            {step == 1 ? (
                <>
                    <div className="d-flex justify-content-center w-100">
                        <h3>{t("Войдите или зарегистрируйтесь")}</h3>
                    </div>

                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="form-group">
                            <div>
                                <input type="text" {...register("username", { required: true })} className={errors.username ? 'has-error' : ''} placeholder={t("Введите номер телефона или почту") + "*"} />
                        ...
                        <div className="form-group mb-1 position-relative">
                            <input type={showPassword ? 'text' : "password"} {...register("password", { required: true })} className={errors.password ? 'has-error' : ''} placeholder={t("Введите пароль")} />
                        ...
                        <div>
                            <a role="button" className="remember-me" onClick={() => setStep(2)}>{t("Не помню пароль")}</a>
                        </div>

                        <div>
                            <button type="submit" className="submit" disabled={isLoading}>{
                                isLoading ? <i className="fa fa-spinner rotating"></i> : t("Войти")
                            }</button>
                        </div>
                    ...
                    <form onSubmit={restoreHandleSubmit(onRestore)}>
                        {step == 2 ? (
                            <>
                                <div className="form-group">
                                    <div>
                                        <input type="text" {...restoreRegister("username", { required: true })} className={restoreErrors.username ? 'has-error' : ''} placeholder={t("Введите номер телефона или почту") + "*"} />
                                ...
                                <div>
                                    <button type="submit" className="submit" disabled={isLoading}>{
                                        isLoading ? <i className="fa fa-spinner rotating"></i> : t("Далее")
                                    }</button>
                                </div>
                            </>) : (step == 3 ? (
                                <>
                                    <div className="form-group">
                                        <div>
                                            <input type='text' autoComplete='off' {...restoreRegister("code", { required: true })} className={restoreErrors.code ? 'has-error' : ''} placeholder={t("Введите полученный код")} />
                                        </div>
                                    </div>
                                        ...
                                        {timeLeft > 0 ? (
                                            ...
                                            <a role="button" className="remember-me" onClick={async () => {
                                                ...
                                                if (res.status == "success") {
                                                    setTimeLeft(180);
                                                    toast.info(t("Повторный код отправлен"))
                                                }
                                    ...
                                    <div>
                                        <button type="submit" className="submit" disabled={isLoading}>{
                                            isLoading ? <i className="fa fa-spinner rotating"></i> : (step == 2 ? t("Далее") : t("Подтвердить"))
                                        }</button>
                                    </div>
                                </>
                            ) : (
                                <>
                                    <div className="form-group position-relative mb-0">
                                        ...
                                        })} className={restoreErrors.password ? 'has-error' : ''} placeholder={t("Введите новый пароль") + "*"} />
                                    ...
                                    <div className="form-group mb-0 mt-3 position-relative">
                                        <input type={showPassword ? 'text' : "password"} {...restoreRegister("repeat_password", {
                                            required: true,
                                            validate: value => value === restorePassword.current || t("Пароли не совпадают")
                                        })} className={restoreErrors.repeat_password ? 'has-error' : ''} placeholder={t("Повторите пароль") + "*"} />
                                        <a role="button" className={"eye " + (showPassword ? 'active' : '')} onClick={() => setShowPassword(!showPassword)}>
                                            {/* Eye svg */}
                                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="14" viewBox="0 0 20 14">
                                                <path d="M10.0007 0.75C5.83398 0.75 2.27565 3.34167 0.833984 7C2.27565 10.6583 5.83398 13.25 10.0007 13.25C14.1673 13.25 17.7256 10.6583 19.1673 7C17.7256 3.34167 14.1673 0.75 10.0007 0.75ZM10.0007 11.1667C7.70065 11.1667 5.83398 9.3 5.83398 7C5.83398 4.7 7.70065 2.83333 10.0007 2.83333C12.3006 2.83333 14.1673 4.7 14.1673 7C14.1673 9.3 12.3006 11.1667 10.0007 11.1667ZM10.0007 4.5C8.61732 4.5 7.50065 5.61667 7.50065 7C7.50065 8.38333 8.61732 9.5 10.0007 9.5C11.384 9.5 12.5007 8.38333 12.5007 7C12.5007 5.61667 11.384 4.5 10.0007 4.5Z" />
                                    ...
                                    <div>
                                        <button type="submit" className="submit" disabled={isLoading}>{
                                            isLoading ? <i className="fa fa-spinner rotating"></i> : t("Сохранить")
                                        }</button>
                                    </div>
                                </>
                            ))}
