...
export default function FeedbackForm({ closeModal }) {
    const globalContext = useContext(GlobalContext);

    const { t, i18n } = useTranslation();

    const defaultValues = {
        full_name: globalContext.store.auth.user?.full_name,
        subject: "",
        phone: "",
        message: ""
    };
    ...
    const onSubmit = async data => {
        try {
            ...
            let res = await post('user/feedback', data);

            if (res.status === 'error') {
                toast.error(t("Произошла ошибка, повторите попытку"), {toastId: "feedback-msg", updateId: "feedback-msg"});
            } else if (res.status == 'success') {
                toast.success(t("Ваше сообщение успешно отправлено"), {toastId: "feedback-msg", updateId: "feedback-msg"});
    ...
    return (
        <div className="rizanova-login transition-all transform">
            ...
            <div className="d-flex justify-content-center w-100">
                <h3>{t("Обратная связь")}</h3>
            </div>

            <form onSubmit={handleSubmit(onSubmit)}>
                <div className="form-group">
                    <div>
                        <input {...register("full_name", { required: true })} className={errors.full_name ? 'has-error' : ''} placeholder={t("Введите ФИО *")} />
                    </div>
                </div>
                <div className="form-group">
                    <div>
                        <input {...register("phone", { required: true })} className={errors.phone ? 'has-error' : ''} placeholder={t("Введите ваш телефон")} />
                    </div>
                </div>
                <div className="form-group">
                    <div>
                        <input {...register("subject", { required: true })} className={errors.subject ? 'has-error' : ''} placeholder={t("Введите тему")} />
                    </div>
                </div>
                <div className="form-group">
                    <div>
                        <textarea {...register("message", { required: true })} className={errors.message ? 'has-error' : ''} placeholder={t("Введите сообщению")} />
                    </div>
                </div>
                <div>
                    <button type="submit" className="submit" disabled={isLoading}>{
                        isLoading ? <i className="fa fa-spinner rotating"></i> : t("Отправить")
                    }</button>
                </div>
            </form>
        </div>
