...
export default function AdvancedDetailPersons({ model, link = "film" }) {
    const {t, i18n} = useTranslation();
    const [showMore, setShowMore] = useState(false);

    return (
        <div className="advanced-person-table person-table">
            <div className="row">
                {/* <div className="col-lg-4">
                    <a href={model.image_path} target="_blank" rel="noreferrer">
                        <img src={model.image_path} className="w-100" alt={model.name} />
                    </a>
                </div> */}
                <div className="col-lg-12">
                    <h1 className="title-1 mt-0">{model.name}</h1>
                    ...
                    <div className="table">
                        ...
                        <Collapse in={showMore}>
                            <div>
                                ...
                                {model.persons?.studios?.length > 0 && <div className="d-flex w-100 justify-content-start align-items-center">
                                    <div className="label">{t("Студия")}:</div>
                                    <div className="value">{model.persons.studios?.map((person, index) => {
                                        return (
                                            <React.Fragment key={index}>
                                                <Link href={`/person/${person.id}`}><a>{person.name}</a></Link>{model.persons.studios?.length - 1 != index && <span className="me-2">,</span>}
                                            </React.Fragment>
                                        );
                                    })}</div>
                                </div>}
