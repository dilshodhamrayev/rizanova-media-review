...
import { SUBSCRIBE } from '../config/index';
...
export default function VideoPlayer({ model, showReview = false }) {
    ...
    useEffect(() => {
        if (player) {
            if (player.currentLevel >= 2) {
                if (!store.auth.isAuth || SUBSCRIBE && store.auth.user?.tariffs.length === 0) {
        ...
        const onKeyDown = (e) => {
            if (e.target.tagName != "INPUT") {
                ...
                if (e.keyCode === 37) { // left
                    if (!firstPlay) {
                        let s = videoPlayer.currentTime - 10;
                        if (s < 0) s = 0;
    ...
    }, [videoPlayer.playing, videoPlayer.duration, videoPlayer.currentTime, firstPlay, playerRef?.current]);

    if (!store.auth.isAuth || SUBSCRIBE && store.auth.isAuth && store.auth.user?.tariffs.length === 0) {
        return (
            <div className="rizanova-video-player not-interactivated">
                <img src={model.image_path} className='w-100 d-block' />
                <div className="first-play">
                    <a role="button" className='big-play-btn btn-big-spec user' onClick={() => {
                        if (!store.auth.isAuth) {
                            toast.info(t("Авторизуйтесь, чтобы посмотреть видео"), { toastId: "watch-video", updateId: "watch-video", /*position: "top-center",*/ hideProgressBar: true });
                            router.push("/login");
                        } else {
                            toast.info(t("Оформите подписку, чтобы посмотреть видео"), { toastId: "watch-video", updateId: "watch-video", /*position: "top-center",*/ hideProgressBar: true });
                        ...
                        <span className='ms-2'>{!store.auth.isAuth ? t("Авторизоваться") : t("Оформить подписку")}</span>
                    </a>
                </div>
            </div>
        );
    ...
    let height = videoPlayer.levels[videoPlayer.levels.length - 1 - (player?.currentLevel != -1 ? player?.currentLevel : 0)]?.height;
        ...
        >
            ...
            <main className="overlay">
                <div className="controls">
                    ...
                    <div className="tools">
                        <a role="button" className="tool video-player-prev-btn" onClick={() => {
                            let s = videoPlayer.currentTime - 10;
                            if (s < 0) s = 0;
                            if (s > videoPlayer.duration) s = videoPlayer.duration;
                            playerRef?.current?.seekTo(s);
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="14" viewBox="0 0 16 15" fill="none">
                                <path fillRule="evenodd" clipRule="evenodd" d="M9.08317 0.0723688C9.01991 0.107367 7.09766 1.57539 4.81149 3.33468C0.225062 6.86416 0.489012 6.63938 0.493383 7.01227C0.497754 7.38496 0.237155 7.16344 4.82792 10.6968C7.23687 12.5509 9.05971 13.9316 9.13983 13.9629C9.45744 14.087 9.8158 13.9582 9.99329 13.6563L10.0855 13.4995L10.1019 11.8933L10.1183 10.2872L12.4351 12.0727C13.7094 13.0547 14.8185 13.8932 14.8998 13.936C15.0205 13.9994 15.0853 14.0115 15.2524 14.0016C15.5037 13.9866 15.6966 13.8658 15.8199 13.6459L15.9021 13.4995V7.00918V0.518869L15.8314 0.38742C15.7924 0.315123 15.7257 0.223404 15.683 0.183608C15.4945 0.00772865 15.1334 -0.044161 14.9067 0.0720074C14.8397 0.106381 13.7348 0.943451 12.4516 1.93218L10.1183 3.72988L10.1019 2.12439L10.0855 0.518869L9.99329 0.36205C9.85882 0.133296 9.68491 0.0298121 9.41544 0.0181789C9.24929 0.0110149 9.17114 0.0237654 9.08317 0.0723688Z" fill="white" />
                            </svg>
                        </a>
                        <a role="button" className={"tool" + (!videoPlayer.playlistToggle ? " video-player-play-btn" : "")} onClick={() => {
                            dispatch({ type: TYPE_VP_SET_PLAYING, payload: !videoPlayer.playing });
                            setSettings(0);
                        }}>
                            {videoPlayer.playing ? (
                        ...
                        <a role="button" className="tool video-player-next-btn" onClick={() => {
                            let s = videoPlayer.currentTime + 10;
                            if (s < 0) s = 0;
                            if (s > videoPlayer.duration) s = videoPlayer.duration;
                            playerRef?.current?.seekTo(s);
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="17" height="14" viewBox="0 0 17 14" fill="none">
                                <path fillRule="evenodd" clipRule="evenodd" d="M0.799656 0.0433331C0.461568 0.14906 0.169504 0.440465 0.0660791 0.775233C0.000920505 0.986259 0 1.0718 0 6.99844C0 12.9377 0.000789006 13.0102 0.0665065 13.223C0.150371 13.4945 0.381549 13.7603 0.646688 13.89C0.884408 14.0063 1.26191 14.0347 1.50348 13.9545C1.63288 13.9115 2.23476 13.4491 4.37588 11.7479C5.86569 10.5641 7.09522 9.59565 7.10817 9.59561C7.12116 9.59558 7.13595 10.4056 7.14105 11.3955L7.15035 13.1954L7.24188 13.3691C7.44426 13.7532 7.78359 13.97 8.21814 13.9926C8.39981 14.0021 8.51043 13.9897 8.63536 13.9457C8.77005 13.8982 9.57611 13.2734 12.5146 10.9386C14.5547 9.31759 16.3216 7.88998 16.4409 7.76611C16.7435 7.45222 16.8139 7.30753 16.8149 6.99844C16.8159 6.68274 16.7389 6.51971 16.4524 6.23031C16.336 6.11275 14.5689 4.68784 12.5256 3.06384C9.84299 0.931686 8.76712 0.0950457 8.65439 0.0533929C8.42009 -0.0331675 8.02773 -0.0119959 7.78655 0.100207C7.5607 0.205276 7.36424 0.395558 7.24188 0.627755L7.15035 0.801468L7.14105 2.60138C7.13595 3.59135 7.12116 4.40127 7.10817 4.40123C7.09522 4.40117 5.86559 3.4328 4.37565 2.24929C2.4065 0.68509 1.62607 0.0836052 1.51788 0.0467521C1.3391 -0.0141656 0.988918 -0.0158094 0.799656 0.0433331Z" fill="white" />
                            </svg>
                        </a>
                        ...
                        <div className="ms-auto d-flex align-items-center">
                            ...
                            {!isIOS && <a role="button" className='tool me-0' onClick={() => {
                                if (screenfull.isEnabled) {
                ...
                <div className={"settings " + (settings >= 1 ? 'show' : '')}>
                    {settings === 1 ? <>
                        {!isIOS && <a role="button" className="d-flex justify-content-between align-items-center setting-item" onClick={() => {
                            setSettings(2);
                        }}>
                            <div className="label">{t("Качество")}</div>
                            <div className="value d-flex align-items-center">
                                <span className="me-1 pe-1 d-flex align-items-center">{videoPlayer.currentLevelIndex === null ? (t("Автонастройка") + " (" + ((!store.auth.isAuth || SUBSCRIBE && store.auth.isAuth && store.auth.user?.tariffs.length === 0 ? (height < 720 ? height : "540") : height) + "p)")) : videoPlayer.levels[videoPlayer.currentLevelIndex]?.height + "p"}</span>
                                    ...
                                    <a role="button" key={index} className="d-flex justify-content-between align-items-center setting-item" onClick={() => {
                                        if (level.height >= 720) {
                                            if (!store.auth.isAuth) {
                                                ...
                                                return;
                                            } else if (SUBSCRIBE && store.auth.user?.tariffs.length === 0) {
                                        ...
                                        <div className="label font-weight-bold d-flex align-items-center">
                                            ...
                                            {level.height >= 720 && (!store.auth.isAuth || SUBSCRIBE && store.auth.isAuth && store.auth.user?.tariffs.length === 0) && <span className="ms-2 fs-12 spec-color"> [{t("Подписка")}] </span>}
                                        </div>
                                    </a>
                                );
                            })}
            ...
            <VideoPlayerPlaylist show={videoPlayer.playlistToggle} goNext={goNext} />
        </div>
    )
}