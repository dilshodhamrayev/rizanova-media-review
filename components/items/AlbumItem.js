...
export default function AlbumItem(props) {
    ...
    const play = () => {
        if (audioPlayer.currentPlaylist_id === "album_" + model.id)
            dispatch({ type: TYPE_AP_SET_PLAYING, payload: !audioPlayer.playing });
        else {
            if (model.musics.length > 0)
                dispatch({ type: TYPE_AP_SET_TRACKS, payload: model.musics, currentTrackId: model.musics[0].id, currentPlaylist_id: "album_" + model.id, currentPlaylist: model.name });
        }
    }
    ...
    return (
        <div className="music-item">
            <div className="image">
                ...
                <div className={"controls " + ((audioPlayer.currentPlaylist_id === "album_" + model.id || isOpenShare) ? 'active' : '')} onClick={(e) => {
                    if (e.target === e.currentTarget) {
                        router.push(`/music/album/${model.id}`);
                    }
                }}>
                    {/* <FavoriteBtn model={model} type="album" /> */}
                    ...
                    {/* <LongShareBtn link={"/album/" + model.id} setIsOpenShare={setIsOpenShare}>
