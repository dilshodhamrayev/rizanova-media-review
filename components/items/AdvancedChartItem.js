...
export default function AdvancedChartItem({ model, order, models, showTrash = false, hideTire = false, mutate }) {
    ...
    return <li className="advanced" onClick={(e) => {
        ...
        <div className="d-flex justify-content-between">
                ...
                <div className="info align-items-center">
                    <div className="artist-name d-block">
                        <a role="button" className={"link" + (audioPlayer.currentTrackId === model.id ? 'active' : '')} onClick={() => {
                            ...
                            <span>{model.name}</span> {model.version && <span className="version">{model.version}</span>} 
                        </a>
                        {model.chart_place && <ChartBadge place={model.chart_place} className='ms-2'/>}
                    </div>
            ...
            <ChartMenu model={model} mutate={mutate} />
